<?php

namespace App\Mcp\Tools\Tags;

use App\Mcp\Concerns\AuthorizesAccess;
use App\Mcp\Concerns\ResolvesLocale;
use App\Repositories\TagRepository;
use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\ResponseFactory;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Read-only. Fetch a single tag by id with its translated fields for the requested locale.')]
class GetTagTool extends Tool
{
    use AuthorizesAccess;
    use ResolvesLocale;

    public function __construct(protected TagRepository $tags) {}

    public function schema(JsonSchema $schema): array
    {
        return [
            'id' => $schema->integer()->description('The tag id.')->required(),
            'locale' => $schema->string()->description('Language code, e.g. "en". Defaults to the site default.'),
        ];
    }

    public function handle(Request $request): Response|ResponseFactory
    {
        if ($denied = $this->deny($request, 'manage_posts')) {
            return $denied;
        }

        $validated = $request->validate([
            'id' => ['required', 'integer'],
            'locale' => ['nullable', 'string', 'max:10'],
        ]);

        $this->applyLocale($validated['locale'] ?? null);

        $tag = $this->tags->getBy('id', $validated['id']);

        if (is_null($tag)) {
            return Response::error("No tag found with id {$validated['id']}.");
        }

        return Response::structured([
            'id' => $tag->id,
            'name' => $tag->name ?? null,
            'slug' => $tag->slug ?? null,
        ]);
    }
}
