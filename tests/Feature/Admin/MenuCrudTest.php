...
class MenuCrudTest extends TestCase
{
    ...
    public function test_admin_can_create_a_menu(): void
    {
        $response = $this->actingAs($this->admin)
            ->post('/laravella-admin/menus/new', $this->payload());
    ...
    {
        $this->actingAs($this->admin)->post('/laravella-admin/menus/new', $this->payload());
        $translation = MenuTranslation::where('title', 'Footer')->firstOrFail();

        $response = $this->actingAs($this->admin)
            ->put('/laravella-admin/menus/' . $translation->menu_id . '/update', $this->payload([
    ...
    public function test_admin_can_delete_a_menu(): void
    {
        ...
        $this->actingAs($this->admin)
            ->delete('/laravella-admin/menus/' . $translation->menu_id . '/delete')
            ->assertOk();
    ...
    {
        $response = $this->actingAs($this->admin)
            ->from('/laravella-admin/menus/new')
            ->post('/laravella-admin/menus/new', ['title' => '']);
    ...
    public function test_user_with_panel_access_but_no_menu_permission_is_blocked(): void
    {
        ...
        $this->actingAs($user)->get('/laravella-admin/menus')->assertStatus(401);
    }
}
