...
# drives it. The dev `larapress` DB is never touched.
#
# Usage:
...
DUSK_DB="${DUSK_DB:-larapress_dusk}"
...
docker compose exec -T mysql sh -c \
    ...
    'CREATE DATABASE IF NOT EXISTS ${DUSK_DB} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; \
     GRANT ALL PRIVILEGES ON ${DUSK_DB}.* TO \"larapress\"@\"%\"; FLUSH PRIVILEGES;'" 2>/dev/null
...
php artisan serve --env=dusk.local --port="${DUSK_PORT}" --no-reload >/tmp/larapress-dusk-serve.log 2>&1 &
SERVE_PID=$!
