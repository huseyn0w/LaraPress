    ...
    <div class="field">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($translation_links as $title => $link)
                <a href="{{env('APP_URL').'/'.$link}}" target="_blank"
                   class="inline-flex items-center gap-1.5 rounded-full border border-ink-200 bg-surface px-3 py-1 text-xs font-medium text-ink-600 transition hover:border-brand-300 hover:text-brand-700">
